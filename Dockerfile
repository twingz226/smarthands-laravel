...
RUN printf '#!/bin/sh\nset -e\necho "==> Preparing environment..."\ncd /var/www/html\nrm -f .env\necho "DB Check: Connection=$DB_CONNECTION Host=$DB_HOST Database=$DB_DATABASE"\nmkdir -p storage/logs storage/framework/cache storage/framework/sessions storage/framework/views bootstrap/cache\nchown -R www-data:www-data storage bootstrap/cache\nchmod -R 775 storage bootstrap/cache\necho "==> Running migrations..."\nMIGRATE_OUTPUT=$(php artisan migrate:fresh --seed --force --no-interaction 2>&1 || true)\necho "$MIGRATE_OUTPUT"\nif echo "$MIGRATE_OUTPUT" | grep -q "Migrating\\|migrated"; then\n  echo "==> Fresh deploy detected, running seeders..."\n  php artisan db:seed --force --no-interaction || echo "Seeding warning"\nelse\n  echo "==> No new migrations, skipping seeders."\nfi\nphp artisan config:cache\nphp artisan route:cache\nphp artisan view:cache\nphp artisan storage:link --force 2>/dev/null || true\necho "==> Testing Nginx config..."\nnginx -t\necho "==> Starting Supervisord..."\nexec /usr/bin/supervisord -c /etc/supervisord.conf\n' > /entrypoint.sh \
